...
import { prospects as mockProspects, type Axis } from "@/lib/mockData";
import type { Prospect } from "@/lib/mockData";
import { getGeneratedLeads, removeGeneratedLead } from "@/lib/generatedLeads";
import { Search, Mail, ExternalLink, Sparkles, X } from "lucide-react";
...
export default function LeadsPage() {
  ...
  const [generated, setGenerated]   = useState<Prospect[]>([]);

  useEffect(() => {
    setGenerated(getGeneratedLeads());
  }, []);

  const allProspects = [...mockProspects, ...generated];
    ...
    <div className="p-8">
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map((p) => {
              const isGenerated = (p as Prospect & { generated?: boolean }).generated;
              return (
                <tr
                  key={p.id}
                  className={`border-b border-slate-50 hover:bg-slate-50/80 transition-colors ${
                    isGenerated ? "bg-amber-50/30" : ""
                  }`}
                >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="flex items-center gap-1.5">
                          <span className="font-semibold text-slate-900 text-xs leading-tight">{p.company}</span>
                          {isGenerated && (
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-[11px] font-semibold px-2 py-1 rounded-full ${stageBadge[p.stage] ?? "bg-slate-100 text-slate-600"}`}>
                      {p.stage}
                    </span>
                  </td>
                  <td className="px-4 py-3">
        ...
        {filtered.length === 0 && (
          <div className="text-center py-14 text-slate-400 text-sm">
            No leads found.{" "}
            <Link href="/generate" className="text-amber-500 hover:underline font-semibold">
              Generate with AI →
            </Link>
          </div>
        )}
